...
from network import MGN, Image_adapter, Image_adapter_77, Image_adapter_77_768, Base_adapter_77_768
...
class Main():
    ...
    def train(self, epoch):
        ...
        for batch, (inputs, labels, captions) in enumerate(self.train_loader):
            ...
            projected_image_embedding = self.image_adapter(embedding_feature)  # (batch, 77, 768) embedding

            # Text embedding
            with torch.no_grad():
                # 입력 캡션 확인
                print("Input captions:", captions[:2])  # 처음 2개의 캡션만 출력
                ...
                # 텍스트 입력 확인
                print("Processed text inputs:", text_inputs.input_ids[:2])  # 처음 2개의 인코딩된 입력 확인
                
                # text_encoder를 통해 직접 텍스트 임베딩 추출
                text_features = self.text_encoder.get_text_features(**text_inputs)  # [batch_size, hidden_size]
                
                # L2 정규화 적용
                cls_embedding = F.normalize(text_features, p=2, dim=-1)
                
            # projected_image_embedding도 정규화
            proj_image_emb = F.normalize(projected_image_embedding[:, 0, :], p=2, dim=-1)
            
            # 손실 계산 및 역전파
            loss, Triplet_Loss, CrossEntropy_Loss, Contrastive_Loss_77 = self.loss(
                outputs, 
                labels, 
                proj_image_emb,  # 정규화된 이미지 임베딩
                cls_embedding    # 정규화된 CLS 토큰
            )
            loss.backward()
            self.optimizer.step()
    ...
    def tsne_visualization(self):
        ...
        # adapter_features가 3차���이라면 mean pooling으로 2차원으로 변환
...
if __name__ == '__main__':
    ...
    if opt.mode == 'train':
        ...
        wandb.init(
            ...
            name=f"only_adapter_blip_0.5_0.5_2.0_margin2_temperature0.20_77_768_cls"
        )
