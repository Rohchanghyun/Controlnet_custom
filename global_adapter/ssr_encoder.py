...
#         self.SSR_layers.load_state_dict(new_odict, strict=False)
    ...
    def get_image_embeds(self, pil_image, num_samples, text, uncond_text):
        ...
        # cond
        clip_image_embeds = self.image_encoder(clip_image, output_hidden_states=True)['hidden_states'][4::4] # 여러 layer의 hidden state 를 가져온다
        clip_image_embeds = torch.cat(clip_image_embeds, dim=1) # 선택한 여러 feature를 concat (concat한 뒤 한번에 transformation, matrix multiplication)
