...
class Main():
    ...
    def train(self, epoch):
        ...
        optimizer = torch.optim.AdamW(self.pipeline.controlnet.parameters(), lr=1e-5)
        ...
        for step, batch in enumerate(tqdm(self.data.sticker_pose_loader)):
            ...
            if step % 100 == 0:
                try:
                    ...
                    with torch.no_grad():
                        test_pose = pose_images[0:1].clone().float()  # float32로 명시적 변환
                        ...
                        print(f"Input pose value range: [{test_pose.min():.2f}, {test_pose.max():.2f}]")
                        print(f"Input pose dtype: {test_pose.dtype}")
                        
                        # 입력 이미지 정규화
                        if test_pose.max() > 1.0:
                            test_pose = test_pose / 255.0
                        ...
                        # 생성된 이미지 값 범위 확인
                        generated_array = np.array(generated_images)
                        print(f"Generated image shape: {generated_array.shape}")
                        print(f"Generated image value range: [{generated_array.min()}, {generated_array.max()}]")
                        
                        # 생성된 이미지가 모두 검은색인지 확인
                        if generated_array.max() == 0:
                            print("Warning: Generated image is completely black!")
                        
                        # 이미지 저장
                        generated_images.save(
                            os.path.join(image_save_dir, f'step_{step}_generated.png')
                        )
                        
                        # 입력 포즈 이미지도 저장해서 확인
                        pose_img = test_pose[0].cpu().numpy().transpose(1, 2, 0)
                        pose_img = (pose_img * 255).astype(np.uint8)
                        Image.fromarray(pose_images).save(
                            os.path.join(image_save_dir, f'step_{step}_input_pose.png')
                        )
                        ...
                        wandb.log({
                            ...
                            "pose_image": wandb.Image(pose_images[0].cpu().float(), caption="Input Pose"),
                            "target_image": wandb.Image(target_images[0].cpu().float(), caption="Target Image")
                        })
