...
if __name__ == '__main__':
    ...
    if opt.mode == 'train':
        ...
        wandb.init(
            ...
            name=f"sd_image_adapter_cfg_77_{opt.epoch}_{opt.lr}"
        )
        for epoch in range(1, opt.epoch + 1):
            ...
            if epoch % 100 == 0:
                ...
                with torch.no_grad():
                    ...
                    uncond_fused = torch.cat([projected_image_embedding, uncond_embeddings], dim=1)
                    
                    # 노이즈에서 이미지 생성
                    latents = torch.randn((1, 4, 64, 64), device='cuda')
                    
                    # 노이즈 스케줄러 타임스텝 설정 수정
                    main.noise_scheduler.set_timesteps(50)  # 타임스텝 수 설정
                ...
                torch.save(main.image_adapter.state_dict(), image_adapter_path)
                ...
                print("\n=== 매칭 실패한 이미지 경로 ===")
                print("Train unmatched:", check_unmatched_images(main.train_loader.dataset))
                print("Query unmatched:", check_unmatched_images(main.query_loader.dataset))
